...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkAccess() {
    if (localStorage.getItem("gamo_trusted_device") === "true") return;

    // Create Overlay
    const overlay = document.createElement("div");
    overlay.id = "access-overlay";
    overlay.style.cssText = `
      position: fixed; inset: 0; background: rgba(5, 5, 5, 0.95); z-index: 999999;
    ...
    container.innerHTML = `
      <h2 style="color:#fff; margin-bottom:10px; font-weight:700;">Restricted Access</h2>
      <p style="color:#aaa; margin-bottom:24px; font-size:0.9rem;">This device is not trusted. Enter the access code to continue.</p>
      <input type="password" id="access-code" placeholder="Enter Access Code" style="
        background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1);
        padding: 12px 16px; border-radius: 10px; color: #fff; width: 100%;
        margin-bottom: 16px; outline: none; text-align: center; letter-spacing: 2px;
      ">
      <button id="access-btn" style="
        background: #8B5CF6; color: #fff; border: none; padding: 12px;
        width: 100%; border-radius: 10px; font-weight: 600; cursor: pointer;
        transition: transform 0.2s;
      ">Verify Device</button>
      <p id="access-error" style="color:#ef4444; margin-top:16px; font-size:0.85rem; display:none;">Invalid Access Code</p>
    `;
    ...
    const errorMsg = container.querySelector("#access-error");
    ...
    const verify = () => {
      ...
      if (code === "HQz7A") {
        ...
        setTimeout(() => btn.style.background = "#8B5CF6", 500);
      }
    };
